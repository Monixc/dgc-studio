...
export default function StudentPortfolioReview() {
  ...
  const revealCompose = useCallback(() => {
    if (!isCompact) return;
  ...
  const addComment = async () => {
    ...
    try {
      ...
      if (submissionIdRef.current !== targetSubmissionId) return;
  ...
  const removeComment = async (commentId: string) => {
    ...
    const targetSubmissionId = submission.id;
    try {
      await deleteComment.mutateAsync({ commentId, submissionId: targetSubmissionId });
      if (submissionIdRef.current !== targetSubmissionId) return;
  ...
  const versionList = (
    <aside className="shrink-0 border-b bg-background lg:flex lg:min-h-0 lg:flex-col lg:border-b-0 lg:border-r">
  ...
  const notePane = (
    <main className={cn("min-w-0 p-3 sm:p-5", isCompact ? "overflow-visible" : "h-full overflow-y-auto")}>
  ...
  const feedbackPane = (
    <aside className={cn(
      ...
      isCompact ? "min-h-0" : "h-full min-h-[24rem] border-l border-t-0",
    )}>
      ...
      <div className={cn("space-y-2 p-3", !isCompact && "flex-1 overflow-y-auto")}>
      ...
      <div
        ...
        className={cn(
          ...
          isCompact && "sticky bottom-0 z-10 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/80",
        )}
      >
    ...
    <div className="flex h-dvh min-h-0 flex-col bg-muted/20">
      <header className="flex shrink-0 items-center gap-2 border-b bg-background p-3">
        ...
        <div className="min-w-0 flex-1">
          <h1 className="truncate text-base font-semibold sm:text-lg">{submission.title || "제목 없는 포트폴리오"}</h1>
      ...
      {isCompact ? (
        ...
        <div className="grid min-h-0 flex-1 overflow-hidden grid-cols-[11rem_minmax(0,1fr)]">
          {versionList}
          <ResizablePanelGroup direction="horizontal" className="min-h-0 min-w-0 overflow-hidden">
