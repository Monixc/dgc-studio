...
>(({ defaultOpen = true, open: openProp, onOpenChange: setOpenProp, className, style, children, ...props }, ref) => {
  ...
  const setOpen = React.useCallback(
    (value: boolean) => (setOpenProp ? setOpenProp(value) : setInternalOpen(value)),
    [setOpenProp]
  );
    ...
    <SidebarContext.Provider value={contextValue}>
      <TooltipProvider delayDuration={0}>
        <div
          ...
          className={cn("group/sidebar-wrapper relative flex h-full w-full transform-gpu", className)}
          {...props}
        >
          {children}
        </div>
  ...
  ({ className, asChild = false, ...props }, ref) => {
    ...
    return (
      <Comp
        ref={ref}
        className={cn(
          "absolute right-3 top-3.5 flex aspect-square w-5 items-center justify-center rounded-md p-0 text-sidebar-foreground outline-none transition-transform hover:bg-sidebar-accent hover:text-sidebar-accent-foreground after:absolute after:-inset-2 md:after:hidden",
...
const sidebarMenuButtonVariants = cva(
  "peer/menu-button flex w-full items-center gap-2 overflow-hidden rounded-md p-2 text-left text-sm outline-none ring-sidebar-ring transition-[width,height,padding] hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2 disabled:pointer-events-none disabled:opacity-50 [&>span:last-child]:truncate [&>svg]:size-4 [&>svg]:shrink-0 group-data-[collapsible=icon]:size-8 group-data-[collapsible=icon]:justify-center group-data-[collapsible=icon]:p-2",
  {
    variants: {
      variant: {
        default: "",
        outline: "border border-sidebar-border bg-background shadow-[0_0_0_1px_var(--sidebar-border)]",
      },
      size: {
        default: "h-9 text-sm",
        sm: "h-7 text-xs",
    ...
    defaultVariants: { variant: "default", size: "default", isActive: false },
  }
);
  ...
  return (
    <Comp
      ...
      className={cn(
        ...
        "group-data-[collapsible=icon]:hidden",
        showOnHover && "opacity-0 group-hover/menu-item:opacity-100 data-[state=open]:opacity-100 peer-data-[active=true]/menu-button:text-sidebar-accent-foreground",
        className
      )}
      {...props}
    />
