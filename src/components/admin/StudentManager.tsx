...
import {
  ...
  SidebarMenu, SidebarMenuItem, SidebarMenuButton, SidebarInset, SidebarTrigger, SidebarRail, SidebarInput,
  useSidebar,
  ...
  if (studentsError) {
    return (
      <div className="flex h-full items-center justify-center p-6">
        <div className="max-w-md rounded-none border border-destructive/30 bg-background p-6 text-center shadow-sm">
    ...
    <SidebarProvider className="h-full min-h-0 items-stretch">
      <Sidebar collapsible="icon" className="border-r">
        <SidebarHeader className="min-h-[45px] border-b">
          ...
          <div className="relative group-data-[collapsible=icon]:hidden">
            ...
            <SidebarInput value={query} onChange={(event) => setQuery(event.target.value)} placeholder="학생 검색" className="pl-8 text-xs" />
          </div>
        </SidebarHeader>
        <SidebarContent>
          <SidebarGroup>
      ...
      <section className="min-w-0 flex-1 overflow-auto bg-muted/20 p-4 md:p-6">
        {!selectedStudent ? <EmptyState /> : <>
          ...
          <Card className="mb-5 p-5 shadow-sm"><div className="mb-4 flex flex-wrap items-center justify-between gap-3"><h3 className="font-semibold">최근 14일 학습 활동</h3><div className="flex items-center gap-2"><div className="flex rounded-md border p-0.5 text-xs"><Button variant="ghost" size="sm" onClick={() => setChartMetric("submissions")} className={cn("h-auto rounded px-2 py-1", chartMetric === "submissions" && "bg-primary text-primary-foreground hover:bg-primary/90")}>제출 횟수</Button><Button variant="ghost" size="sm" onClick={() => setChartMetric("correct")} className={cn("h-auto rounded px-2 py-1", chartMetric === "correct" && "bg-primary text-primary-foreground hover:bg-primary/90")}>정답 문제</Button></div><div className="flex rounded-md border p-0.5 text-xs"><Button variant="ghost" size="sm" onClick={() => setChartType("bar")} className={cn("h-auto rounded px-2 py-1", chartType === "bar" && "bg-primary text-primary-foreground hover:bg-primary/90")}>막대</Button><Button variant="ghost" size="sm" onClick={() => setChartType("line")} className={cn("h-auto rounded px-2 py-1", chartType === "line" && "bg-primary text-primary-foreground hover:bg-primary/90")}>선</Button></div></div></div><ActivityChart data={dailyData} max={maxDaily} type={chartType} metric={chartMetric} /></Card>
...
function Metric({ label, value, icon: Icon }: { label: string; value: string; icon: typeof BookOpen }) { return <div className="rounded-none border bg-background p-3"><Icon className="mb-2 size-4 text-primary" /><p className="text-[11px] text-muted-foreground">{label}</p><p className="mt-0.5 text-lg font-bold">{value}</p></div>; }
...
function StudentMenuItem({
  student, isActive, onSelect,
}: {
  student: { id: string; display_name: string; classes: { name: string }[] };
  isActive: boolean;
  ...
  return (
    <SidebarMenuItem>
      <SidebarMenuButton
        isActive={isActive}
        tooltip={student.display_name || "이름 없음"}
        className="h-auto py-2 group-data-[collapsible=icon]:!size-8 group-data-[collapsible=icon]:!p-1"
        onClick={() => {
          onSelect(student.id);
